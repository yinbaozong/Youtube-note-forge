...
test("start job marks active, health, and RPC requests with the extension identity", async () => {
  ...
  const worker = createWorker(async (url, options = {}) => {
    ...
    if (url.endsWith("/health")) return jsonResponse({ status: "ok", version: "4.0.3", current_vault: "C:\\Vault" });
    return jsonResponse({
      type: "accepted",
      status: "running",
      stage: "credentials",
...
test("settings never report connected when the authenticated RPC is rejected", async () => {
  const worker = createWorker(async (url) => {
    if (url.endsWith("/health")) return jsonResponse({ status: "ok", version: "4.0.3" });
    return jsonResponse({
      type: "error",
      status: "error",
