...
test("client identity binds every local request to the running extension version", () => {
  assert.equal(
    buildClientIdentity("abcdefghijklmnopabcdefghijklmnop", "4.1.1"),
    "abcdefghijklmnopabcdefghijklmnop@4.1.1"
  );
  assert.throws(() => buildClientIdentity("", "4.1.1"), /扩展 ID/);
});
...
test("plugin settings prefer RPC values and fall back to health", () => {
  assert.deepEqual(
    normalizePluginSettings(
      { status: "ok", model: "health/model", default_vault: "C:\\Vault", version: "4.1.1" },
...
test("health alone never reports a green connection without a successful settings RPC", () => {
  assert.deepEqual(
    normalizePluginSettings({ status: "ok", version: "4.1.1" }, {}),
    {
      connected: false,
      model: "未配置",
      vault: "未配置",
